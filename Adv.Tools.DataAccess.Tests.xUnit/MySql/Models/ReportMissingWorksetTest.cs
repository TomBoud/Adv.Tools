...
{
    public class ReportMissingWorksetTest
    {
        ...
        {
            //Stage
            var task = _access.DeleteTableIfExists(TestDataBaseName, nameof(ReportMissingWorkset));
            //Act
            await task;
            //Assert
        ...
        {
            //Stage
            var query = new ReportMissingWorkset().GetCreateTableQuery(TestDataBaseName);
        ...
        {
            //Stage
            var models = new List<ReportMissingWorkset>
            {
                new ReportMissingWorkset()
                {
                    Id =0,
                    ModelName = "testModel",
                    ModelGuid = "testGuid",
        ...
        {
            //Stage
            var task = _access.LoadDataSelectAllAsync<ReportMissingWorkset>(TestDataBaseName);
            //Act
